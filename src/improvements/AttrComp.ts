...
export interface PlayerAttrType {
	[key: string]: number | string;
};

export interface PlayerAttrInsideType {
	Main: {
		[key: string]: number | string;
	}
	Effect: {
		[key: number]: EffectAttr
	}
	EffectSuit: string[];
};

export function GetPlayerAttr(player: JPlayer): PlayerAttrType;
export function GetPlayerAttr(player: JPlayer, mode: 1): PlayerAttrInsideType;
export function GetPlayerAttr(player: JPlayer, mode: number, key: string): number;
/**
 * 获取玩家属性
 ...
 */
export function GetPlayerAttr(player: any, mode?: any, key?: any): any{
...
interface EffectAttr {
	id: number|false;
	level: number;
	time: number;
}

export function SetPlayerAttr(player: string, item: string, newAttr: object): void;
export function SetPlayerAttr(player: JPlayer, item: string, newAttr: object): void;
/**
 * 设置玩家属性
 ...
 * @returns 
 */
export function SetPlayerAttr(player: any, item: string, newAttr: any) {
    /** 玩家名字 */
    let name = "";
	if (isPlayer(player)) {
		name = player.getName().toLocaleLowerCase();
	...
	switch (item) {
		case "Effect": {//newAttr: {id: 1, level: 1, time: s};
			let newAttr_ = <EffectAttr>newAttr;
			if (newAttr_.id === false) {
				data["Effect"] = {};
				return true;
			}
			let effectdata = data["Effect"][newAttr_.id];
			if (!effectdata) {
				effectdata = {level: 0, time: 0};
			}
			if (newAttr_.level && newAttr_.level > effectdata.level) {
				effectdata.level = newAttr_.level;
			}
			if (newAttr_.time) {
				effectdata.time = Number((new Date().getTime()/1000).toFixed(0)) + Number(newAttr_.time);
				data.Effect[newAttr_.id] = effectdata;
			} else {
				delete data.Effect[newAttr_.id];
			}
		...
		case "Main": {
			console.warn("setPlayerAttr(): '"+item+"' is the wrong item parameter.");
			return false;
		}
	}
	if (newAttr['delete']) {
