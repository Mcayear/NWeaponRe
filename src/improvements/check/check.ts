...
export function checkAttr(sender: cn.nukkit.Player, p: cn.nukkit.Player, type: string, data: any): void;
 ...
 * @param {any} [data] 玩家属性数据，只有当 /nwe check attr 时传入
 * @returns 
 */
            ...
            for (let i in data.Main) {
                ...
                switch (i) {// 主属性中的绿值显示
                    case '血量值': {
                        ...
                        str += "§a+" + (parseInt(value) * parseInt(valueToString(data.Main['生命加成']))).toFixed(0) + "§r\n";
                        break;
                    }
                    case '攻击力': {
                        if (!data.Main['攻击加成']) {
