...
const PlayerDeathData = new Map(), PlayerAttkCool = new Map();
...
pnx.listenEvent("cn.nukkit.event.entity.EntityDamageByEntityEvent", EventPriority.LOW, event => {
    ...
    if (event.getEventName() === "cn.nukkit.event.entity.EntityDamageEvent" || event.getEventName() === "cn.nukkit.event.entity.EntityDamageByBlockEvent")
        return;
    ...
    if (Damager instanceof LittleNpcClass) {
        let it = Damager.getConfig().getMonsterAttrMap().entrySet().iterator();
        while (it.hasNext()) {
            let entry = it.next();
            let key = entry.getKey();
            let value = entry.getValue();
            ...
            if (typeof (value) === 'number') {
                ...
                DAttr[key] = getRandomNum(value);
            }
        }
    }
    else {
        DAttr = isPlayer(Damager) ? GetPlayerAttr(Damager) : (NMonster ? NMonster.GetMonsterAttr(Damager.getId(), 0, false) : {});
    }
    // 实体名字格式化
    if (isPlayer(Wounded)) {
    ...
    if (FinalDamage > 0) {
        if (isPlayer(Wounded)) {
            ...
            PlayerDeathData.set(Wounded.name, [DamagerName, isPlayer(Damager) ? blockitem.getItemInHand(Damager).getCustomName() : null]);
...
/** 在某些时候，会重复触发，所以间隔3s
var DeathTipsTimeout = new Map();*/
pnx.listenEvent("cn.nukkit.event.player.PlayerDeathEvent", EventPriority.LOW, event => {
    ...
    if (PlayerDeathData.has(player.name)) {
        let [damager, name] = PlayerDeathData.get(player.name);
        let tips = "{damager} 击败了 {player}";
        if (_C.WeaponConfig[name] && _C.WeaponConfig[name].击杀提示) {
            tips = _C.WeaponConfig[name].击杀提示;
        }
        event.setDeathMessage(tips.replace("{damager}", damager).replace("{name}", name).replace("{player}", player.name));
        PlayerDeathData.delete(player.name);
    }
});
