import { defineData, getProbabilisticResults, getRandomNum } from "../util/Tool.js";
        ...
        default:
            {
                ...
                let oldLore = blockitem.getItemLore(HandItem), probability = Config.Strengthen.chance[nbtObj.level] + defineData(strengthFailedNum[player.name]) * Config.Strengthen.failedAddition, failProtect = 0, straightUp = 0, luck = false;
...
export function ForgingFakeInvChange(event) {
    ...
    if (!PlayerSmithingTempData.has(player.name)) {
        PlayerSmithingTempData.set(player.name, null);
    }
    if (slot == 0) {
        let paperData = PaperConfig[newItem.getCustomName()];
        if (paperData) {
            if (!PlayerData[player.getName()])
                PlayerData[player.getName()] = { exp: 0, level: 0 };
