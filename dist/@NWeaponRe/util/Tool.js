...
export function getItem(name, data) {
    ...
    switch (data.类型) {
        ...
        case "锻造石": {
            lore = "§r" + data.介绍;
            if (data.强度)
                item.getNamedTag().putString('Strength', data.强度);
            break;
        }
        case "精工石":
        case "宝石券":
        case "强化石": {
            lore = "§r" + data.介绍;
            if (data.直升)
                item.getNamedTag().putString('StraightUp', data.直升);
            if (data.幸运)
                item.getNamedTag().putString('Luck', data.幸运);
            if (data.堆叠使用)
                item.getNamedTag().putString('stacking', "1");
            if (data.失败保护)
                item.getNamedTag().putString('FailProtect', data.失败保护);
            break;
        }
        case "符文": {
            lore = [
...
export function onlyNameGetItem(type, itemname, count, sender) {
    ...
    switch (type) {
        ...
        default: {
            if (sender)
                sender.sendMessage("[NWeapon] §cUnknowed Item Type:§7 " + type);
            return null;
        }
    }
    if (obj === undefined || obj === null) {
        if (sender)
            sender.sendMessage("[NWeapon] " + type + "物品 " + itemname + " §r不存在");
        return null;
    }
    else {
        ...
        if (typeof (item) === "undefined") {
            if (sender)
                sender.sendMessage("[NWeapon] " + type + "物品 " + itemname + " §r配置文件有误");
            return null;
        }
...
export function valueToString(data, i) {
    ...
    if (_C.MainConfig.AttrDisplayPercent.indexOf(i) > -1) {
        let tempDataStr = (valueData * 100).toFixed(2) + [];
...
export function getProbabilisticResults(value) {
    ...
    if (Number((Math.random() + "").substring(3, length + 3)) - 0 + 1 > value)
        return false;
    return true;
}
/**
 ...
 * @returns
 */
export function itemUnbindPlayer(item, player) {
 ...
 * @returns
 */
export function itemBindPlayer(item, player, noTips, onlyBind) {
    let name = player.getName();
    if (!item.getNamedTag().getString('PlayerBind')) {
 ...
 */
export function examineNeed(needArray, inv, isCheck, player) {
    ...
    for (var i = 0; i < needArray.length; i++) {
        ...
        let info = arr[1];
        let count = 1;
        let itemDataArr = arr[0].split(":"); // Item Data.
        if (type[0] === "item") {
            ...
            tag1: if (item) {
                ...
                for (let v of [...inv.getContents().values()]) {
...
*/
export function randChooes(num, arr1, arr2) {
    if (arr1.length !== arr2.length)
        throw "The length of arr1 is not equal to the length of arr2";
    if (num < 1)
        throw "num is not allowed less than 1";
    if (num >= arr2.length)
        return arr1;
    const res = [];
    for (let i = 0; i < num; i++) {
