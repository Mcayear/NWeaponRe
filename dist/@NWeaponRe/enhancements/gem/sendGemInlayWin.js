...
export function sendGemInlayWin(player) {
    ...
    let nbtStr = HandItem.getNamedTag().getString('GemList');
    let nbtObj;
    if (nbtStr === '') {
        ...
        for (var i = 0; i < 9; i++) {
            ...
            if (item.getCustomName() && item.getNamedTag().getString('NWeaponNameTag')) {
                ...
                if (arr[0] === _C.ItemTypeList["宝石券"]) {
                    let count = 1;
                    let luck_ = Number(item.getNamedTag().getString('Luck')) || 0;
                    let failProtect_ = Number(item.getNamedTag().getString('FailProtect')) || 0;
                    if (luck && luck_) {
                        continue;
                    }
                    else if (luck_ > 0) {
