...
async function start() {
    const { version, getGradeSymbol, _C, TaskExecList, getItData, getArmorConfig, getGemConfig, getJewelryConfig, getPaperConfig, getRuneConfig, getWeaponConfig } = await import('./util/WeaponConfig.js');
    ...
    // 仅存储在内存的数据
    var ShowObj = new Map();
    var PlayerShowCount = new Map();
    if (!contain('seikoFailedNum')) { // 精工失败次数 记录
        exposeObject('seikoFailedNum', new Map());
    }
    if (!contain('strengthFailedNum')) { // 强化失败次数 记录
        exposeObject('strengthFailedNum', new Map());
    }
    /**
     * 添加延时任务
    ...
    function NWeaponCommandHandle(_cmd, _ori, out, res) {
        ...
        const sender = _ori.type === 0 ? _ori.player._PNXEntity : server.getConsoleSender();
        switch (res.action) {
            case "addnbt": {
                if (!sender.isOp()) {
                ...
                if (data) {
                    ...
                    if (befor) {
                        ...
                        _C.MainConfig.unbind.succExec.forEach(function (v) {
                            server.dispatchCommand(server.getConsoleSender(), v.replace("%p", target.getName()));
                        });
                        return;
                    }
                }
            ...
            case "dz": {
                sender.addWindow(new inventory().addInv(false, Java.to([], "cn.nukkit.item.Item[]"), "锻造", ForgingFakeInvChange));
                break;
            }
            case "seiko": {
            ...
            case "show": {
                if (res.showId) {
                     ...
                     * 分享的物品数据
                     */
                    let data = ShowObj.get(res.showId);
                    if (data === undefined) {
                ...
                if (PlayerShowCount.get(sender.getName()) > vipLv) {
                ...
                if (item.getId() == 0)
                    return out.addMessage("[NWeapon] 你分享了份空气 : )");
                ...
                if (!PlayerShowCount.has(ShowObj.get(uid).player)) {
                    ...
                    PlayerShowCount.set(ShowObj.get(uid).player, PlayerShowCount.get(ShowObj.get(uid).player) + 1);
                }
                // 一分钟后销毁分享
            ...
            case "reload": {
                ...
                _C.MainConfig = JSON.parse(Util.YAMLtoJSON(File.readFrom("./plugins/NWeapon/Config.yml")));
            ...
            case "fixtag": {
                ...
                if (handItemName.substring(0, 2) === "§r" && handItemLore.indexOf("§r§4§l一一一一一一一一一一") > -1) {
                    ...
                    if (data) {
                        handItem.getNamedTag().putString('NWeaponNameTag', _C.ItemTypeList[data.类型] + ";" + handItemName);
    ...
    mc.listen("onServerStarted", () => {
        let cmd = mc.newCommand("nwe", "NWeapon 年系列装备插件", PermType.Any);
