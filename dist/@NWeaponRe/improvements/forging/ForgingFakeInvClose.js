...
function minusArray(arr1, arr2) {
    ...
    var temp = [], tempArr = Tool.cloneObjectFn(arr1);
 ...
 * @returns {number}
 */
function getForgingAttr(index, value) {
    /** 获取共享变量 */
    var _C = contain('NWeapon_C');
...
function WeaponToForgeEntry(player, item, index, Strength, PaperData, data) {
    ...
    var _C = contain('NWeapon_C');
    const AttrLore = [];
    let ForgingAttrNbt = { quality: 0, mAttr: {}, attr: {}, info: { player: "", intensity: 0, playerlv: 0 } };
    let mainAttr = data[0];
    ...
    for (let i in ubAttr) {
        if (typeof (ubAttr[i]) === 'string') {
            ...
            ForgingAttrNbt.attr[i] = [getForgingAttr(index, ubAttr[i])];
        }
...
function WeaponToForge(player, item, index, Strength, PaperData, data) {
    ...
    for (let i in data.锻造属性) {
        if (typeof (data.锻造属性[i]) === 'string') {
            ...
            ForgingAttrNbt.attr[i] = [getForgingAttr(index, data.锻造属性[i])];
        }
        AttrLore.push("§r§a" + i + ":§d " + Tool.valueToString(ForgingAttrNbt.attr[i], i));
    }
    let 强度 = "||||||||||||||||||||".split("");
